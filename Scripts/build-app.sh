...
FINAL_APP_DIR="$PROJECT_DIR/dist/YTMusic.app"
FINAL_ZIP="$PROJECT_DIR/dist/YTMusic.zip"
BUILD_WORK="$(mktemp -d)"
trap '/bin/rm -rf "$BUILD_WORK"' EXIT
APP_DIR="$BUILD_WORK/YTMusic.app"
CONTENTS_DIR="$APP_DIR/Contents"
MACOS_DIR="$CONTENTS_DIR/MacOS"
RESOURCES_DIR="$CONTENTS_DIR/Resources"

case "$FINAL_APP_DIR" in
  "$PROJECT_DIR"/dist/YTMusic.app) ;;
  *) print -u2 "Refusing to build outside the project dist directory"; exit 1 ;;
esac

case "$FINAL_ZIP" in
  "$PROJECT_DIR"/dist/YTMusic.zip) ;;
  *) print -u2 "Refusing to archive outside the project dist directory"; exit 1 ;;
esac

cd "$PROJECT_DIR"
swift build -c release --arch arm64 --arch x86_64 --product YTMusic
BIN_DIR="$(swift build -c release --arch arm64 --arch x86_64 --show-bin-path)"

/bin/mkdir -p "$MACOS_DIR" "$RESOURCES_DIR"
/usr/bin/ditto "$BIN_DIR/YTMusic" "$MACOS_DIR/YTMusic"
/usr/bin/ditto "$PROJECT_DIR/Support/Info.plist" "$CONTENTS_DIR/Info.plist"

RESOURCE_BUNDLE="$BIN_DIR/YTMusic_YTMusic.bundle"
if [[ -d "$RESOURCE_BUNDLE" ]]; then
  /usr/bin/ditto "$RESOURCE_BUNDLE" "$RESOURCES_DIR/YTMusic_YTMusic.bundle"
fi
...
/bin/mkdir -p "$ICONSET"
/usr/bin/sips -s format png "$PROJECT_DIR/Sources/YTMusic/Resources/AppIcon.svg" --out "$ICON_WORK/base.png" >/dev/null
